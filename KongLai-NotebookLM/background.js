...
const RULE_SETS_KEY = "automationRuleSets";
...
const EXECUTION_SETTINGS_KEY = "automationExecutionSettings";
...
const DEFAULT_SOURCE_REFRESH_LABELS = Object.freeze([
  "点击即可与 Google 云端硬盘同步",
  "与 Google 云端硬盘同步",
  "云端硬盘同步",
  "同步",
...
const DEFAULT_EXECUTION_SETTINGS = Object.freeze({
  sourceConcurrency: 2,
  retryCount: 1,
...
const activeNotebookRuns = new Set();
const pendingNotebookRuns = new Set();
...
function normalizeExecutionSettings(raw = {}) {
  return {
    sourceConcurrency: clampInteger(raw?.sourceConcurrency, DEFAULT_EXECUTION_SETTINGS.sourceConcurrency, 1, 6),
    retryCount: clampInteger(raw?.retryCount, DEFAULT_EXECUTION_SETTINGS.retryCount, 0, 3),
    retryBaseDelayMs: clampInteger(raw?.retryBaseDelayMs, DEFAULT_EXECUTION_SETTINGS.retryBaseDelayMs, 200, 5000)
  };
}

async function getExecutionSettings() {
  const data = await chrome.storage.local.get(EXECUTION_SETTINGS_KEY);
  const normalized = normalizeExecutionSettings(data?.[EXECUTION_SETTINGS_KEY] || DEFAULT_EXECUTION_SETTINGS);
  await chrome.storage.local.set({ [EXECUTION_SETTINGS_KEY]: normalized });
  return normalized;
}

async function saveExecutionSettings(patch = {}) {
  const current = await getExecutionSettings();
  const next = normalizeExecutionSettings({
    ...current,
    ...patch
  });
  await chrome.storage.local.set({ [EXECUTION_SETTINGS_KEY]: next });
  return next;
}
...
async function syncAlarm(rule = null) {
  ...
  const ruleSets = await getRuleSets().catch(() => [buildDefaultRuleSetEntity()]);
  const ruleSetMap = new Map((ruleSets || []).map((item) => [item.id, item]));
  const targetIntervals = sortedTargets(effectiveRule)
    .map((target) => {
      const matchedRuleSet = getRuleSetByTarget(ruleSetMap, target);
      return normalizeRuleSetIntervalMinutes(matchedRuleSet?.intervalMinutes, effectiveRule.intervalMinutes);
    })
    .filter((value) => Number.isFinite(value) && value > 0);
  const minIntervalMinutes = targetIntervals.length
    ? Math.min(...targetIntervals)
    : normalizeRuleSetIntervalMinutes(effectiveRule.intervalMinutes, DEFAULT_RULE.intervalMinutes);
  const tickMinutes = toAlarmTickMinutes(minIntervalMinutes);
...
async function buildStateSnapshot() {
  ...
  const activeAndPendingNotebookRuns = [...new Set([...activeNotebookRuns, ...pendingNotebookRuns])];
  return {
    rule: state.rule,
    runtime: state.runtime,
    executionSettings,
    currentRunState: currentRunState ? structuredClone(currentRunState) : null,
    activeNotebookRuns: activeAndPendingNotebookRuns,
    alarm: alarm
      ? {
...
function notebookDomAutomation(payload) {
  ...
  const refreshNeedles = [
    ...(Array.isArray(payload?.refreshLabels) ? payload.refreshLabels : [payload?.refreshLabel]),
    ...DEFAULT_SOURCE_REFRESH_LABELS
  ]
...
async function runDomAutomation(tabId, payload) {
  const injected = await chrome.scripting.executeScript({
    ...
    args: [{
      ...
      refreshLabel: payload.refreshLabel,
      refreshLabels: Array.isArray(payload?.refreshLabels) ? payload.refreshLabels : []
    }]
  });
  return injected?.[0]?.result || { ok: false, result: "page_error", stage: "inject", message: "Injected script returned no result." };
}

function getRuleSetByTarget(ruleSetMap = new Map(), target = {}) {
  const ruleSetId = String(target?.ruleSetId || DEFAULT_RULE_SET_ID).trim() || DEFAULT_RULE_SET_ID;
  return ruleSetMap.get(ruleSetId) || ruleSetMap.get(DEFAULT_RULE_SET_ID) || null;
}

function getTargetRunKey(target = {}) {
  const ruleSetId = String(target?.ruleSetId || DEFAULT_RULE_SET_ID).trim() || DEFAULT_RULE_SET_ID;
  ...
  return `${ruleSetId}|${notebookUrl}|${sourceLabel}`;
}

function shouldRunTarget(rule, runtime, target, mode, ruleSetMap = new Map()) {
  if (mode === "manual") return true;
  if (!rule.enabled) return false;
  const ruleSet = getRuleSetByTarget(ruleSetMap, target);
  const intervalMinutes = normalizeRuleSetIntervalMinutes(ruleSet?.intervalMinutes, rule.intervalMinutes);
  const targetKey = getTargetRunKey(target);
  const normalizedUrl = normalizeNotebookUrl(target?.notebookUrl || "", "");
  const lastRunAt = runtime.lastRunAtByTarget?.[targetKey]
    || runtime.lastRunAtByUrl?.[normalizedUrl]
    || "";
  return minutesSince(lastRunAt) >= intervalMinutes;
}
...
function sortedTargets(rule) {
  ...
  for (const target of (rule.targets || [])) {
    const key = `${target?.ruleSetId || "scheduled_source_refresh"}|${target?.notebookUrl || ""}|${target?.sourceLabel || ""}`.trim();
...
function parseRefreshLabelCandidates(...values) {
  const output = [];
  ...
  return dedupeStrings([...output, ...DEFAULT_SOURCE_REFRESH_LABELS]).slice(0, 12);
}

function inferSourceKind(source = {}) {
  const mime = String(source?.sourceMimeType || "").trim().toLowerCase();
  const url = String(source?.sourceUrl || "").trim().toLowerCase();
  const name = String(source?.name || source?.sourceName || "").trim().toLowerCase();

  if (mime.includes("google-apps.document") || url.includes("docs.google.com/document")) return "gdoc";
  if (mime.includes("google-apps.spreadsheet") || url.includes("docs.google.com/spreadsheets")) return "gsheet";
  if (
    ...
    || /\.(xlsx?|csv|tsv)(?:$|[?#])/i.test(url)
    || /\.(xlsx?|csv|tsv)$/i.test(name)
  ) {
    return "excel";
  }
  if (String(source?.driveFileId || source?.gDocId || "").trim()) return "gdrive";
  return "web";
}

function isRefreshableSource(source = {}) {
  const sourceId = String(source?.id || source?.sourceId || "").trim();
  const driveFileId = String(source?.driveFileId || source?.gDocId || "").trim();
  const sourceKind = String(source?.sourceKind || inferSourceKind(source)).trim().toLowerCase();
  if (!sourceId || !driveFileId) return false;
  return sourceKind === "gdoc" || sourceKind === "gsheet" || sourceKind === "excel" || sourceKind === "gdrive";
}
...
function normalizeSourceRecord(source = {}) {
  ...
  return {
    ...
    driveFileId,
    sourceKind,
    isRefreshable: isRefreshableSource({ ...source, driveFileId, sourceKind }),
    isGDoc: sourceKind === "gdoc"
  };
}

function compareRuleSetPriority(left, right) {
  const leftSystem = Boolean(left?.isSystem);
  const rightSystem = Boolean(right?.isSystem);
  if (leftSystem !== rightSystem) return leftSystem ? 1 : -1;

  const leftInterval = normalizeRuleSetIntervalMinutes(left?.intervalMinutes, DEFAULT_RULE.intervalMinutes);
  const rightInterval = normalizeRuleSetIntervalMinutes(right?.intervalMinutes, DEFAULT_RULE.intervalMinutes);
  if (leftInterval !== rightInterval) return leftInterval - rightInterval;
  ...
  const updatedDiff = rightUpdated.localeCompare(leftUpdated);
  if (updatedDiff !== 0) return updatedDiff;

  return String(left?.name || left?.id || "").localeCompare(String(right?.name || right?.id || ""));
}

function buildNotebookRunTasks(rule, runtime, mode, targetUrlFilter, ruleSetMap = new Map()) {
  const grouped = new Map();
  const normalizedFilter = normalizeNotebookUrl(targetUrlFilter, "");

  for (const target of sortedTargets(rule)) {
    const notebookUrl = normalizeNotebookUrl(target?.notebookUrl || "", "");
    if (!notebookUrl) continue;
    if (normalizedFilter && !compareNotebookUrls(notebookUrl, normalizedFilter)) continue;
    if (!grouped.has(notebookUrl)) {
      grouped.set(notebookUrl, {
        notebookUrl,
        targets: []
      });
    }
    grouped.get(notebookUrl).targets.push(target);
  }

  const tasks = [];
  grouped.forEach((bucket, notebookUrl) => {
    const ruleSets = [];
    const seenRuleSetIds = new Set();
    for (const target of bucket.targets) {
      const ruleSet = getRuleSetByTarget(ruleSetMap, target) || buildDefaultRuleSetEntity();
      const ruleSetId = String(ruleSet?.id || DEFAULT_RULE_SET_ID).trim() || DEFAULT_RULE_SET_ID;
      if (seenRuleSetIds.has(ruleSetId)) continue;
      seenRuleSetIds.add(ruleSetId);
      ruleSets.push(ruleSet);
    }
    ruleSets.sort(compareRuleSetPriority);
    const effectiveIntervalMinutes = ruleSets.length
      ? Math.min(...ruleSets.map((item) => normalizeRuleSetIntervalMinutes(item?.intervalMinutes, rule.intervalMinutes)))
      : normalizeRuleSetIntervalMinutes(rule.intervalMinutes, DEFAULT_RULE.intervalMinutes);

    const shouldRun = mode === "manual"
      || (
        rule.enabled
        && minutesSince(runtime.lastRunAtByUrl?.[notebookUrl] || "") >= effectiveIntervalMinutes
      );
    if (!shouldRun) return;

    tasks.push({
      notebookUrl,
      targets: bucket.targets.slice(),
      ruleSets,
      ruleSetIds: ruleSets.map((item) => String(item?.id || DEFAULT_RULE_SET_ID).trim() || DEFAULT_RULE_SET_ID),
      ruleSetNames: ruleSets.map((item) => String(item?.name || "Rule Set").trim() || "Rule Set"),
      sourceLabels: dedupeStrings(bucket.targets.map((item) => item?.sourceLabel)),
      refreshLabels: parseRefreshLabelCandidates(ruleSets.map((item) => item?.refreshLabel), rule.refreshLabel),
...
function classifyFailureReason(message = "", result = "page_error") {
  const text = String(message || "").trim().toLowerCase();
  if (!text && result) return String(result);
  if (result === "skipped" || text.includes("not refreshable")) return "not_refreshable";
  if (result === "login_required" || text.includes("login") || text.includes("permission") || text.includes("access required")) {
    return "login_permission";
  }
  if (text.includes("timeout")) return "timeout";
  if (text.includes("quota") || text.includes("limit")) return "quota_limit";
  if (text.includes("source not found")) return "source_not_found";
  if (text.includes("refresh entry not found") || text.includes("refresh_not_found")) return "refresh_not_found";
  if (text.includes("rpc")) return "rpc_failed";
  if (text.includes("network")) return "network";
  if (text.includes("page")) return "page_error";
  return String(result || "unknown").trim() || "unknown";
}

function createEmptyRunSummary(mode = "manual") {
  return {
...
function updateCurrentRunState(summary = null) {
  if (!summary) {
  ...
  currentRunState = {
    ...summary,
    updatedAt: nowIso(),
    activeNotebookUrls: [...activeNotebookRuns]
  };
}

function pushFailureReason(target, reason) {
  if (!reason) return;
  target.failureReasons[reason] = Number(target.failureReasons[reason] || 0) + 1;
}

function pushFailedSample(target, sample) {
  if (!sample || !sample.sourceName) return;
  if (target.failedSamples.length >= 3) return;
  target.failedSamples.push({
    notebookUrl: sample.notebookUrl || "",
    notebookTitle: sample.notebookTitle || "",
    sourceId: sample.sourceId || "",
    sourceName: sample.sourceName || "",
    sourceKind: sample.sourceKind || "",
    result: sample.result || "page_error",
    reason: sample.reason || ""
  });
}

function summarizeRunResult(summary = {}) {
  if (summary.failedCount > 0 && (summary.successCount > 0 || summary.skippedCount > 0)) return "partial_success";
  if (summary.failedCount > 0) return "page_error";
  if (summary.successCount > 0) return "success";
  if (summary.skippedCount > 0) return "skipped";
  return "idle";
}

function buildNotebookRunMessage(task, notebookTitle, notebookSummary) {
  const ruleSummary = task.ruleSetNames.length ? task.ruleSetNames.join(" + ") : "Rule Set";
  return [
    notebookTitle || task.notebookUrl,
    `sources=${notebookSummary.totalSources}`,
    `success=${notebookSummary.successCount}`,
    `failed=${notebookSummary.failedCount}`,
    `skipped=${notebookSummary.skippedCount}`,
    `rules=${ruleSummary}`
  ].join(" | ");
}
...
async function runWithConcurrency(items, concurrency, worker) {
  ...
  const limit = Math.max(1, Math.min(6, parseIntSafe(concurrency, 2)));
  let index = 0;

  const runners = new Array(Math.min(limit, normalizedItems.length)).fill(null).map(async () => {
...
async function createNotebookDomRefreshRunner(notebookUrl, runtime, refreshLabels) {
  let prepared = false;
  let tabId = 0;

  return async (source) => {
    if (!prepared) {
      const dedicatedTab = await getDedicatedNotebookTab(notebookUrl, runtime);
      runtime.dedicatedTabs[notebookUrl] = dedicatedTab.id;
      const loadedTab = await loadNotebookIntoDedicatedTab(dedicatedTab.id, notebookUrl);
      const tabIssue = classifyTabUrl(loadedTab.url);
      if (tabIssue) {
        return { ok: false, targetUrl: notebookUrl, ...tabIssue };
      }
      tabId = dedicatedTab.id;
      prepared = true;
    }

    const response = await runDomAutomation(tabId, {
      sourceLabel: String(source?.name || source?.sourceName || "").trim() || "work ai news",
      refreshLabel: refreshLabels?.[0] || "",
      refreshLabels
    });
    ...
    if (!response?.ok) {
      return {
        ok: false,
        targetUrl: notebookUrl,
        result: response?.result || "page_error",
        message: response?.message || "NotebookLM page automation failed."
      };
    }
    ...
    return {
      ...
      targetUrl: notebookUrl,
      result: "success",
      message: response?.message || "Refresh action clicked."
    };
  };
}

async function refreshSourceWithRetry({
  notebookId,
  notebookUrl,
  notebookTitle,
  source,
  ...
  const sourceName = String(source?.name || source?.sourceName || "").trim() || "Untitled Source";
  const sourceKind = String(source?.sourceKind || inferSourceKind(source)).trim() || "web";
  const sourceId = String(source?.id || source?.sourceId || "").trim();
  ...
  if (!isRefreshableSource(source)) {
    return {
      ...
      skipped: true,
      result: "skipped",
      message: `${sourceName} is not refreshable by current automation.`,
  ...
  const attempts = Math.max(1, Number(executionSettings?.retryCount || 0) + 1);
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      await runWithAuthUsers(async (client) => {
        await client.syncGdocSource(notebookId, sourceId);
        return true;
      });
      return {
        ok: true,
      ...
      try {
        const domResult = await domRefresh(source);
        if (domResult?.ok) {
          return {
            ok: true,
            skipped: false,
            result: "success",
            message: domResult?.message || `DOM refreshed ${sourceName}.`,
            reason: "",
            strategy: "dom",
            sourceId,
            sourceName,
        ...
        lastFailure = new Error(domResult?.message || rpcError?.message || "source_refresh_failed");
      } catch (domError) {
        lastFailure = domError instanceof Error ? domError : new Error(String(domError || rpcError?.message || "source_refresh_failed"));
      }

      if (attempt < attempts) {
        const backoffMs = Math.max(200, Number(executionSettings?.retryBaseDelayMs || DEFAULT_EXECUTION_SETTINGS.retryBaseDelayMs))
          * (2 ** (attempt - 1));
  ...
  const failureMessage = String(lastFailure?.message || "source_refresh_failed").trim();
  return {
    ok: false,
    skipped: false,
    result: "page_error",
    message: failureMessage,
    reason: classifyFailureReason(failureMessage, "page_error"),
    strategy: "rpc_dom_failed",
    sourceId,
    sourceName,
    sourceKind,
    notebookUrl,
...
async function runNotebookTask(task, runtime, executionSettings, runSummary, mode) {
  const notebookStartedAt = Date.now();
  const notebookUrl = normalizeNotebookUrl(task?.notebookUrl || "", "");
  if (!notebookUrl) throw new Error("invalid_notebook_url");

  const payload = await fetchNotebookSources({ notebookUrl, force: true });
  const notebookId = String(payload?.notebookId || "").trim();
  const notebookTitle = String(payload?.notebookTitle || "").trim();
  const sources = (Array.isArray(payload?.sources) ? payload.sources : []).map((item) => normalizeSourceRecord(item));
  const domRefresh = await createNotebookDomRefreshRunner(notebookUrl, runtime, task.refreshLabels);
  const logEntries = [];
  ...
  runSummary.totalSources += sources.length;
  updateCurrentRunState(runSummary);

  await runWithConcurrency(sources, executionSettings?.sourceConcurrency, async (source) => {
    const sourceResult = await refreshSourceWithRetry({
      notebookId,
      notebookUrl,
      notebookTitle,
      source,
    ...
    if (sourceResult.skipped) {
      notebookSummary.skippedCount += 1;
      runSummary.skippedCount += 1;
    } else if (sourceResult.ok) {
      notebookSummary.successCount += 1;
      runSummary.successCount += 1;
    } else {
      notebookSummary.failedCount += 1;
      runSummary.failedCount += 1;
      pushFailureReason(notebookSummary, sourceResult.reason);
      pushFailureReason(runSummary, sourceResult.reason);
      pushFailedSample(notebookSummary, sourceResult);
      pushFailedSample(runSummary, sourceResult);
    }

    runSummary.completedSources += 1;
    updateCurrentRunState(runSummary);
    ...
    logEntries.push({
      ...
      message: `${notebookTitle || notebookUrl} | ${sourceResult.sourceName} | ${sourceResult.strategy} | ${sourceResult.message}`,
      notebookUrl,
      notebookTitle,
      ...
      ruleSetIds: task.ruleSetIds,
      ruleSetNames: task.ruleSetNames
    });
  });

  notebookSummary.durationMs = Date.now() - notebookStartedAt;
  notebookSummary.result = summarizeRunResult(notebookSummary);
  ...
  logEntries.push({
    ...
    mode,
    scope: "notebook",
    result: notebookSummary.result,
    message: buildNotebookRunMessage(task, notebookTitle, notebookSummary),
    notebookUrl,
    notebookTitle,
    sourceKind: "notebook",
    durationMs: notebookSummary.durationMs,
    runId: runSummary.runId,
    ruleSetIds: task.ruleSetIds,
    ruleSetNames: task.ruleSetNames,
    stats: {
      total: notebookSummary.totalSources,
      success: notebookSummary.successCount,
      failed: notebookSummary.failedCount,
      skipped: notebookSummary.skippedCount
    }
...
function pickRefreshableSources(sources = []) {
  return (Array.isArray(sources) ? sources : [])
    .map((item) => normalizeSourceRecord(item))
    .filter((item) => item.isRefreshable);
}
...
async function performRun(mode = "manual", targetUrlFilter = "") {
  ...
  const tasks = buildNotebookRunTasks(rule, runtime, mode, targetUrlFilter, ruleSetMap);
  if (!tasks.length) return buildStateSnapshot();

  const runSummary = createEmptyRunSummary(mode);
  runSummary.totalNotebooks = tasks.length;
  updateCurrentRunState(runSummary);
  ...
  try {
    ...
    for (const task of tasks) {
      ...
      activeNotebookRuns.add(notebookUrl);
      updateCurrentRunState(runSummary);

      try {
        const notebookResult = await runNotebookTask(task, runtime, executionSettings, runSummary, mode);
        const finishedAt = notebookResult.finishedAt || nowIso();
        runtime.lastRunAtByUrl[notebookUrl] = finishedAt;
        if (!runtime.lastRunAtByTarget || typeof runtime.lastRunAtByTarget !== "object") {
          runtime.lastRunAtByTarget = {};
        }
        task.targets.forEach((target) => {
          const runKey = getTargetRunKey(target);
          if (runKey) runtime.lastRunAtByTarget[runKey] = finishedAt;
        });
        notebookResult.logEntries.forEach((entry) => {
          runtime = appendRunLog(runtime, entry);
        });
        if (notebookResult.failedCount > 0) {
          failures.push({
            targetUrl: notebookUrl,
            result: notebookResult.result,
            message: buildNotebookRunMessage(task, notebookResult.notebookTitle, notebookResult)
          });
        }
      } catch (error) {
        const failureMessage = error?.message || "NotebookLM refresh execution failed.";
        failures.push({
          targetUrl: notebookUrl,
          result: "page_error",
          message: failureMessage
        ...
        runSummary.failedCount += 1;
        runSummary.completedSources += 1;
        pushFailureReason(runSummary, classifyFailureReason(failureMessage, "page_error"));
        pushFailedSample(runSummary, {
          notebookUrl,
          notebookTitle: "",
          sourceId: "",
          sourceName: notebookUrl,
          sourceKind: "notebook",
          result: "page_error",
        ...
        runtime = appendRunLog(runtime, {
          ...
          scope: "notebook",
          result: "page_error",
          message: `${notebookUrl} | sources=0 | success=0 | failed=1 | skipped=0 | rules=${task.ruleSetNames.join(" + ") || "Rule Set"} | ${failureMessage}`,
          notebookUrl,
          notebookTitle: "",
          sourceKind: "notebook",
          durationMs: 0,
        ...
        activeNotebookRuns.delete(notebookUrl);
        runSummary.completedNotebooks += 1;
        updateCurrentRunState(runSummary);
      }
    }
  } finally {
    runSummary.finishedAt = nowIso();
    runSummary.durationMs = Date.now() - startedAtMs;
    runSummary.result = summarizeRunResult(runSummary);
  }
  ...
  runtime.lastRunSummary = {
    ...
    totalNotebooks: runSummary.totalNotebooks,
    completedNotebooks: runSummary.completedNotebooks,
    totalSources: runSummary.totalSources,
    completedSources: runSummary.completedSources,
    successCount: runSummary.successCount,
    failedCount: runSummary.failedCount,
    skippedCount: runSummary.skippedCount,
    failureReasons: runSummary.failureReasons,
    failedSamples: runSummary.failedSamples
  };
  if (mode === "manual") {
    runtime.lastManualRunSummary = runtime.lastRunSummary;
  }
  if (mode === "scheduled") {
    runtime.lastScheduledRunSummary = runtime.lastRunSummary;
  }

  if (!failures.length && runSummary.failedCount <= 0) {
    runtime.lastResult = runSummary.result === "skipped" ? "success" : runSummary.result;
    runtime.lastSuccessAt = nowIso();
    runtime.lastErrorMessage = "";
    runtime.lastNotifiedErrorKey = "";
  } else {
    const firstFailure = failures[0] || {
      targetUrl: runSummary.failedSamples[0]?.notebookUrl || "",
      result: runSummary.result,
      message: runSummary.failedSamples[0]?.reason || "NotebookLM refresh execution failed."
    };
    runtime.lastResult = runSummary.result;
    runtime.lastErrorMessage = `${firstFailure.targetUrl} | ${firstFailure.message}`.trim();
...
async function runNotebookNow(url = "") {
  ...
  if (activeNotebookRuns.has(normalizedUrl) || pendingNotebookRuns.has(normalizedUrl)) {
  ...
  pendingNotebookRuns.add(normalizedUrl);
  try {
    return await enqueueRun("manual", normalizedUrl);
  } finally {
    pendingNotebookRuns.delete(normalizedUrl);
  }
}
...
async function saveRule(nextRule) {
  ...
  const ruleSets = await getRuleSets();
  const validRuleSetIds = new Set((ruleSets || []).map((item) => item.id));
  normalizedRule.targets = normalizeTargetsByRuleSets(normalizedRule.targets || [], validRuleSetIds);
...
function normalizeRuleSetIntervalMinutes(value, fallback = DEFAULT_RULE.intervalMinutes) {
  const parsed = parseIntSafe(value, fallback);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.max(5, Math.min(1440, parsed));
}

function normalizeRuleSetRefreshLabel(value, fallback = DEFAULT_RULE.refreshLabel) {
  return String(value || fallback || "").trim().slice(0, 120);
}

function buildDefaultRuleSetEntity() {
  return {
    id: DEFAULT_RULE_SET_ID,
    name: "Scheduled Source Refresh",
    category: RULE_SET_CATEGORY_SCHEDULED_REFRESH,
    description: "",
    intervalMinutes: normalizeRuleSetIntervalMinutes(DEFAULT_RULE.intervalMinutes, 60),
    refreshLabel: normalizeRuleSetRefreshLabel(DEFAULT_RULE.refreshLabel, ""),
    isSystem: true,
    updatedAt: nowIso(),
    createdAt: nowIso()
  };
...
function normalizeRuleSet(raw = {}) {
  const id = String(raw.id || makeEntityId("ruleset")).trim() || makeEntityId("ruleset");
  const normalizedName = String(raw.name || "").trim().slice(0, 80);
  const isSystem = Boolean(raw.isSystem) || id === DEFAULT_RULE_SET_ID;
  const createdAt = String(raw.createdAt || "").trim() || nowIso();
  const updatedAt = String(raw.updatedAt || "").trim() || createdAt;
  return {
    id,
    name: normalizedName || (isSystem ? "Scheduled Source Refresh" : "Rule Set"),
    category: normalizeRuleSetCategory(raw.category || raw.type || ""),
    description: String(raw.description || "").trim().slice(0, 240),
    intervalMinutes: normalizeRuleSetIntervalMinutes(raw.intervalMinutes, DEFAULT_RULE.intervalMinutes),
    refreshLabel: normalizeRuleSetRefreshLabel(raw.refreshLabel, DEFAULT_RULE.refreshLabel),
    isSystem,
    updatedAt,
    createdAt
  };
...
function sortRuleSets(list = []) {
  return [...list].sort((left, right) => {
    const leftSystem = left?.id === DEFAULT_RULE_SET_ID || left?.isSystem;
    const rightSystem = right?.id === DEFAULT_RULE_SET_ID || right?.isSystem;
    if (leftSystem !== rightSystem) return leftSystem ? -1 : 1;
    return String(right?.updatedAt || "").localeCompare(String(left?.updatedAt || ""));
  });
}

function ensureDefaultRuleSet(list = []) {
  const normalized = [];
  const seen = new Set();
  for (const item of list) {
    const next = normalizeRuleSet(item);
    if (seen.has(next.id)) continue;
    seen.add(next.id);
    normalized.push(next);
  }
  if (!seen.has(DEFAULT_RULE_SET_ID)) {
    normalized.unshift(buildDefaultRuleSetEntity());
  }
  return sortRuleSets(normalized);
}

async function getRuleSets() {
  const data = await chrome.storage.local.get(RULE_SETS_KEY);
  const list = Array.isArray(data[RULE_SETS_KEY]) ? data[RULE_SETS_KEY] : [];
  const output = ensureDefaultRuleSet(list).slice(0, 200);
  await chrome.storage.local.set({ [RULE_SETS_KEY]: output });
  return output;
}

function normalizeTargetsByRuleSets(targets = [], validRuleSetIds = new Set([DEFAULT_RULE_SET_ID])) {
  const seen = new Set();
  const output = [];
  for (const rawTarget of Array.isArray(targets) ? targets : []) {
    const notebookUrl = normalizeNotebookUrl(rawTarget?.notebookUrl || "", "");
    const sourceLabel = String(rawTarget?.sourceLabel || "").trim();
    if (!notebookUrl || !sourceLabel) continue;
    const maybeRuleSetId = String(rawTarget?.ruleSetId || DEFAULT_RULE_SET_ID).trim() || DEFAULT_RULE_SET_ID;
    const ruleSetId = validRuleSetIds.has(maybeRuleSetId) ? maybeRuleSetId : DEFAULT_RULE_SET_ID;
    const key = `${ruleSetId}|${notebookUrl}|${sourceLabel}`;
    if (seen.has(key)) continue;
    seen.add(key);
    output.push({ notebookUrl, sourceLabel, ruleSetId });
  }
  return output;
}

async function saveRuleSet(payload = {}) {
  const normalized = normalizeRuleSet(payload);
  const current = await getRuleSets();
  ...
  if (foundIndex >= 0) {
    next[foundIndex] = {
      ...next[foundIndex],
      ...normalized,
      isSystem: next[foundIndex].id === DEFAULT_RULE_SET_ID || next[foundIndex].isSystem,
      createdAt: next[foundIndex].createdAt || normalized.createdAt || now,
      updatedAt: now
    };
  } else {
    next.unshift({
  ...
  const persisted = ensureDefaultRuleSet(next).slice(0, 200);
  await chrome.storage.local.set({ [RULE_SETS_KEY]: persisted });
  await syncAlarm();
  const ruleSet = persisted.find((item) => item.id === normalized.id) || persisted[0];
  return { ruleSets: persisted, ruleSet, snapshot: await buildStateSnapshot() };
}

function pickFallbackRuleSetId(ruleSets = [], preferredId = "", removedId = "") {
  const ids = (ruleSets || []).map((item) => String(item?.id || "").trim()).filter(Boolean);
  const preferred = String(preferredId || "").trim();
  if (preferred && preferred !== removedId && ids.includes(preferred)) return preferred;
  if (ids.includes(DEFAULT_RULE_SET_ID)) return DEFAULT_RULE_SET_ID;
  return ids.find((id) => id !== removedId) || DEFAULT_RULE_SET_ID;
}

async function deleteRuleSet(id = "", fallbackRuleSetId = "") {
  const targetId = String(id || "").trim();
  if (!targetId) throw new Error("rule_set_id_required");
  if (targetId === DEFAULT_RULE_SET_ID) throw new Error("cannot_delete_default_rule_set");

  const current = await getRuleSets();
  const exists = current.some((item) => item.id === targetId);
  if (!exists) return { ruleSets: current, fallbackRuleSetId: DEFAULT_RULE_SET_ID, snapshot: await buildStateSnapshot() };

  const filtered = current.filter((item) => item.id !== targetId);
  const normalizedRuleSets = ensureDefaultRuleSet(filtered).slice(0, 200);
  const fallbackId = pickFallbackRuleSetId(normalizedRuleSets, fallbackRuleSetId, targetId);

  await chrome.storage.local.set({ [RULE_SETS_KEY]: normalizedRuleSets });

  const validRuleSetIds = new Set(normalizedRuleSets.map((item) => item.id));
  await mutateState((state) => {
    const normalizedRule = normalizeRule(state.rule || DEFAULT_RULE);
    const retargeted = (normalizedRule.targets || []).map((target) => ({
      ...target,
      ruleSetId: String(target?.ruleSetId || DEFAULT_RULE_SET_ID).trim() === targetId
        ? fallbackId
        : String(target?.ruleSetId || DEFAULT_RULE_SET_ID).trim()
    }));
    normalizedRule.targets = normalizeTargetsByRuleSets(retargeted, validRuleSetIds);
    state.rule = normalizedRule;
    return state;
  });
  await syncAlarm();
...
async function applyTemplateToNotebooks({ templateId, notebookUrls }) {
  ...
  const existingKey = new Set((rule.targets || []).map((item) => `${item.ruleSetId || DEFAULT_RULE_SET_ID}|${item.notebookUrl}|${item.sourceLabel}`));
  let added = 0;
  for (const notebookUrl of normalizedUrls) {
    for (const sourceLabel of template.sourceLabels) {
      const key = `${DEFAULT_RULE_SET_ID}|${notebookUrl}|${sourceLabel}`;
      if (existingKey.has(key)) continue;
      existingKey.add(key);
      rule.targets.push({ notebookUrl, sourceLabel, ruleSetId: DEFAULT_RULE_SET_ID });
      added += 1;
    }
  }
...
async function getManagerMeta() {
  const [favorites, ruleSets, collections, templates, notebookTags, audioTasks, sourceMeta, localUser] = await Promise.all([
    getFavorites(),
    getRuleSets(),
    getCollections(),
    getTemplates(),
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === "SAVE_EXECUTION_SETTINGS") {
    saveExecutionSettings(message?.payload || {})
      .then(async (executionSettings) => ({
        executionSettings,
        snapshot: await buildStateSnapshot()
      }))
      .then((payload) => sendResponse({ ok: true, ...payload }))
      .catch((error) => sendResponse({ ok: false, error: error?.message || "save_execution_settings_failed" }));
    return true;
  }
  ...
  if (message?.type === "SAVE_RULE_SET") {
    saveRuleSet(message?.payload || {})
      .then((payload) => sendResponse({ ok: true, ...payload }))
      .catch((error) => sendResponse({ ok: false, error: error?.message || "save_rule_set_failed" }));
    return true;
  }

  if (message?.type === "DELETE_RULE_SET") {
    deleteRuleSet(message?.id || "", message?.fallbackRuleSetId || "")
      .then((payload) => sendResponse({ ok: true, ...payload }))
      .catch((error) => sendResponse({ ok: false, error: error?.message || "delete_rule_set_failed" }));
    return true;
  }
