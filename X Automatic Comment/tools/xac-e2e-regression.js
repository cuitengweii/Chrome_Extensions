...
async function ensurePanelOpen(page) {
  await page.waitForSelector('#xac-root', { timeout: START_TIMEOUT_MS })
  const collapsed = await page.$eval('#xac-root', (el) => el.classList.contains('collapsed')).catch(() => false)
  if (collapsed) {
    await page.click('#xac-t').catch(() => {})
    await sleep(250)
  }
  await page.waitForSelector('#xac-run-toggle', { timeout: START_TIMEOUT_MS })
}
...
async function gotoXPage(page) {
  const urls = ['https://x.com/explore', 'https://x.com/home', 'https://x.com']
  let lastError
